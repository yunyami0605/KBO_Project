...
def render_correlation_section():
    ...
    m1.metric("피어슨 상관계수", f"{stats.get('pearson_correlation', 0):.4f}")
    m2.metric("상관관계 강도", stats.get('pearson_interpretation', "-"))
    m3.metric("샘플 수", f"{stats.get('sample_size', 0)}개")

    # 6) 통계 유의성 표시
    if stats.get('pearson_p_value', 1) < 0.05:
    ...
    st.markdown(f"**스피어만 상관계수:** {stats.get('spearman_correlation', 0):.4f}")
    ...
    if selected_years > 1:
        ...
        if selected_team == "전체 구단":
            trend_df = trend_df.groupby('year')\
    ...
    # 11) 분석 인사이트
    st.markdown("###  분석 인사이트")
    corr_val = stats.get('pearson_correlation', 0)
    if abs(corr_val) >= 0.3:
        if corr_val > 0:
